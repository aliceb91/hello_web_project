...
def test_sort_list_of_names(web_client):
    # Given a list of names
    # It returns the list in alphabetical order.
    response = web_client.post('/sort-names', data={'names': 'Joe,Alice,Zoe,Julia,Kieran'})
    assert response.status_code == 200
    assert response.data.decode('utf-8') == 'Alice,Joe,Julia,Kieran,Zoe'

def test_return_updated_name_list(web_client):
    # Given multiple names as a query
    # It returns an alphabatised list of names with the new names added.
    response = web_client.get('/names?names=Alice,Jess')
    assert response.status_code == 200
    assert response.data.decode('utf-8') == 'Alice,Ben,Catherine,Jess,Zainab'